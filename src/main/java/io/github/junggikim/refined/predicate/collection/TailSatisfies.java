...
public final class TailSatisfies<T> implements Constraint<List<T>> {

    private final Constraint<List<T>> delegate;

    public TailSatisfies(Predicate<List<T>> predicate) {
        Predicate<List<T>> checked = Objects.requireNonNull(predicate, "predicate");
        this.delegate = RefinedSupport.require(
            "tail-satisfies",
            "the tail slice must satisfy the predicate",
            value -> !value.isEmpty() && checked.test(RefinedSupport.copyListSlice(value, 1, value.size()))
        );
    }
